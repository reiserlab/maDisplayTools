...
classdef PanelsControllerNative < handle
    % PanelsControllerNative TCP implementation using MATLAB native tcpclient
    %
    % Drop-in replacement for PanelsController using MATLAB's built-in
    % tcpclient instead of pnet MEX. Requires MATLAB R2014b+.
    %
    % See also: PanelsController
    ...
    properties
        hostName = PanelsControllerNative.defaultHostName;
        port = PanelsControllerNative.defaultPort;
        tcpConn = [];  % tcpclient object (empty when not connected)
    ...
    methods 
        ...
        function self = PanelsControllerNative(varargin)
            if numel(varargin) > 0
        ...
        function open(self, startHost)
            ...
            % running. Disconnect via PanelsControllerNative.close.
            %
            % see also close
            arguments
                self (1,1) PanelsControllerNativeNative
            ...
            fprintf("tcp connection opened (tcpclient)\n");
        end
        ...
        function close(self, stopHost)
            ...
            % PanelsControllerNative.open. If stopHost is true then also stop the
            % `G4 Host.exe`.
            %
            % see also open
            arguments
        ...
        function isOpen = get.isOpen(self)
            ...
            % Host.exe is active.
            isOpen = ~isempty(self.tcpConn) && isa(self.tcpConn, 'tcpclient');
        end
        ...
        function rtn = setRootDirectory(self, dirName, createDir)
            ...
            arguments
                self (1,1) PanelsControllerNative
                dirName (1,1) string
