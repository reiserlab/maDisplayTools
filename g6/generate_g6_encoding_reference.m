...
%% Generate CRC test vectors (CRC-8/AUTOSAR + CRC-16/CCITT-FALSE)
fprintf('Generating CRC test vectors...\n');

crc_vectors = struct();
crc_vectors.crc8_autosar = struct( ...
    'polynomial', '0x2F', ...
    'init', '0xFF', ...
    'refin', false, ...
    'refout', false, ...
    'xorout', '0xFF', ...
    'spec_section', 'g6_01-panel-protocol.md § CRC-8 algorithm', ...
    'usage', 'G6 .pat header byte 17 over header bytes 0-16');

crc_vectors.crc8_autosar.vectors = { ...
    struct('name', 'universal_check', 'input_ascii', '123456789', 'expected_hex', '0xDF'); ...
    struct('name', 'cipo_2L_oneshot_zeros', ...
           'description', '2L Oneshot all-zero payload, parity cleared (53 bytes: 0x01, 0x10, 51 zeros)', ...
           'expected_hex', sprintf('0x%02X', crc8_autosar([uint8(hex2dec('01')), uint8(hex2dec('10')), zeros(1,51,'uint8')]))); ...
    struct('name', 'cipo_16L_oneshot_zeros', ...
           'description', '16L Oneshot all-zero payload, parity cleared (203 bytes: 0x01, 0x30, 201 zeros)', ...
           'expected_hex', sprintf('0x%02X', crc8_autosar([uint8(hex2dec('01')), uint8(hex2dec('30')), zeros(1,201,'uint8')]))) ...
};

crc_vectors.crc16_ccitt_false = struct( ...
    'polynomial', '0x1021', ...
    'init', '0xFFFF', ...
    'refin', false, ...
    'refout', false, ...
    'xorout', '0x0000', ...
    'spec_section', 'g6_04-pattern-file-format.md § Frame Format / Per-frame CRC-16', ...
    'usage', 'G6 .pat per-frame trailer (little-endian); scope = FR_magic + frame_index + panel_blocks');

crc_vectors.crc16_ccitt_false.vectors = { ...
    struct('name', 'universal_check', 'input_ascii', '123456789', 'expected_hex', '0x29B1'); ...
    struct('name', 'all_zero_4bytes', ...
           'description', 'Frame header for frame_index=0 (4 bytes: FR_magic + 0x00 0x00)', ...
           'expected_hex', sprintf('0x%04X', crc16_ccitt_false([uint8('FR'), uint8(0), uint8(0)]))) ...
};

ref.crc_test_vectors = crc_vectors;

%% Generate canonical pattern bytes (full file SHA-256) for cross-language byte-equivalence checks
fprintf('Generating canonical pattern checksums...\n');
% Build small canonical patterns and pin their SHA-256s.
canonical = struct();

% G6_2x10 GS2, 1 frame, mod(i+j,2) checker
canonPats = zeros(40, 200, 1, 'uint8');
for i = 1:40, for j = 1:200, canonPats(i,j,1) = mod(i+j, 2); end, end
canon_save_dir = tempname; mkdir(canon_save_dir);
g6_save_pattern(canonPats, uint8(1), [2, 10], canon_save_dir, 'canon_2x10_gs2_1f', 'Mode', 'GS2', 'Overwrite', true);
canon_path = fullfile(canon_save_dir, 'canon_2x10_gs2_1f_G6.pat');
fid_c = fopen(canon_path, 'r'); canon_bytes = fread(fid_c, Inf, 'uint8=>uint8'); fclose(fid_c);
canonical.g6_2x10_gs2_1f_checker = struct( ...
    'description', 'G6_2x10 GS2, 1 frame, pixel(i,j)=mod(i+j,2) (1-indexed)', ...
    'file_size', length(canon_bytes), ...
    'first_24_bytes_hex', join(string(arrayfun(@(b) sprintf('%02X', b), canon_bytes(1:24), 'UniformOutput', false)), ' '), ...
    'sha256_hex', sha256_hex(canon_bytes));

ref.canonical_patterns = canonical;

%% Write JSON file
fprintf('Writing JSON file: %s\n', output_file);

% Write tight JSON (the naive pretty-printer below corrupted strings that
% contained literal '{' or '}' — switched to PrettyPrint mode if available,
% else tight). Both are valid JSON; consumers parse them fine.
try
    json_str = jsonencode(ref, 'PrettyPrint', true);
catch
...
%% Helper: SHA-256 of a uint8 vector, hex string
function h = sha256_hex(bytes)
    md = java.security.MessageDigest.getInstance('SHA-256');
    md.update(uint8(bytes));
    digest = md.digest();
    % Convert signed Java bytes to unsigned + hex
    h = '';
    for k = 1:length(digest)
        b = digest(k);
        if b < 0, b = b + 256; end
        h = [h sprintf('%02x', b)];
    end
