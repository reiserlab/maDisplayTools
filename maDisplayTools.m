...
%                                     end
        ...
        function [frames, meta] = load_pat_g6(filepath)
            ...
            % Parse G6 header (17 bytes)
            version = data(5);
            if version ~= 1
                warning('Unknown G6 version: %d (expected 1)', version);
            end

            gs_val_g6 = data(6);  % 1 = GS2, 2 = GS16
            num_frames = data(7) + data(8) * 256;
            row_count = data(9);
            col_count = data(10);
            checksum = data(11);
            panel_mask = data(12:17);
            ...
            % Frame data starts after 17-byte header
            frame_data = data(18:end);
            offset = 1;
        ...
        function [frames, meta] = preview_pat(filepath, show_plot)
            ...
            %
            %   Usage:
            %       [frames, meta] = maDisplayTools.preview_pat('path/to/pattern.pat')  % Shows plot
            %       [frames, meta] = maDisplayTools.preview_pat('path/to/pattern.pat', true)
            %       [frames, meta] = maDisplayTools.preview_pat('path/to/pattern.pat', false)  % No plot
        ...
        %   [NumPatsX, NumPatsY, gs_val, RowN, ColN, raw] = maDisplayTools.read_header_and_raw('pattern.pat');
            ...
            fid = fopen(path, 'rb');
            if fid == -1
                error('maDisplayTools:FileNotFound', 'Could not open file: %s', path);
            end
        ...
        %
        % EXAMPLE:
        %   frame_bytes = maDisplayTools.frame_size_bytes(4, 12, 4);
            ...
            elseif gs_val == 4 || gs_val == 16
                ...
                error('maDisplayTools:InvalidGrayscale', ...
        ...
        %
        % EXAMPLE:
        %   dims = maDisplayTools.get_pattern_dimensions('pattern.pat');
        %   fprintf('Pattern has %d rows and %d columns\n', dims.num_rows, dims.num_cols);
            ...
            [NumPatsX, NumPatsY, gs_val, RowN, ColN, ~] = ...
                maDisplayTools.read_header_and_raw(pattern_path);
        ...
        %
        % EXAMPLE:
        %   [is_valid, error_msg] = maDisplayTools.validate_pattern_dimensions('pattern.pat', 4, 12);
        %   if ~is_valid
        %       fprintf('Error: %s\n', error_msg);
        %   end
            ...
            if ~isfile(pattern_path)
                error('maDisplayTools:FileNotFound', 'Pattern file not found: %s', pattern_path);
            end
            
            % Try to read pattern dimensions
            try
                dims = maDisplayTools.get_pattern_dimensions(pattern_path);
            catch ME
                error('maDisplayTools:ReadFailed', ...
