classdef test_script_plugin < matlab.unittest.TestCase
    ...
    %
    % Tests initialization, execution, error handling, and cleanup of
    % the ScriptPlugin class using temporary dummy scripts.
    %
    % Note: ScriptPlugin.initialize() is now called automatically from the
    % constructor (it became a private method). Tests no longer call
    ...
    methods (Test)
        function testInitializeWithValidFunction(testCase)
            % Test ScriptPlugin construction with valid dummy function
            % (initialize is called automatically from constructor)
        ...
        function testNonexistentScriptPath(testCase)
            % Test ScriptPlugin with nonexistent script path -> error during construction
            % (initialize is called from constructor, so error occurs at construction time)
            definition = struct();
            definition.type = 'script';
            definition.script_path = fullfile(testCase.testRoot, 'nonexistent_script.m');
            definition.config = struct('experimentDir', testCase.testRoot);

            % Should error during construction since initialize() runs in constructor
            testCase.verifyError(...
                @() ScriptPlugin('bad_path_test', definition, testCase.logger), ...
                'ScriptPlugin:FileNotFound', ...
        ...
        function testCleanupRemovesPath(testCase)
            ...
            % Verify path was added (initialize ran in constructor)
