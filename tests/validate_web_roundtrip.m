...
%           details (cell array): detailed results for each test
    ...
    webPatsDir = fullfile(maDisplayToolsRoot, 'tests', 'web_generated_patterns');
    manifestFile = fullfile(webPatsDir, 'web_generated_manifest.json');

    if ~exist(manifestFile, 'file')
        error(['Manifest not found: %s\n' ...
               'Run: node tests/generate-roundtrip-patterns.js --outdir ../maDisplayTools/tests/web_generated_patterns'], ...
               manifestFile);
    end
