    ...
    if options.ValidateDriveName
        try
            ...
            if ~contains(vol_name, 'PATSD')
                mapping.error = sprintf(['SD card on %s: is not named PATSD.\n' ...
                    'Found: %s\n' ...
                    'To fix: right-click drive in Explorer > Properties > rename to PATSD,\n' ...
                    'or format with: format %s: /FS:FAT32 /V:PATSD /Q\n' ...
                    'Or pass ''ValidateDriveName'', false to skip this check.'], ...
                    sd_drive, strtrim(vol_name), sd_drive);
                return;
            end
