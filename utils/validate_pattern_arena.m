...
%
% Inputs:
%   pattern_path      - Path to pattern .pat file
...
%     patterns/G6_2x10/my_pattern_G6.pat
%     patterns/G41_2x12_cw/my_pattern_G4.pat
%
...
%   [valid, info] = validate_pattern_arena('patterns/G6_2x10/grating_G6.pat');
%
%   % Validate pattern against explicit arena config
%   [valid, info] = validate_pattern_arena('my_pattern.pat', 'configs/arenas/G6_2x10.yaml');
%
% See also: load_arena_config, maDisplayTools.load_pat

%% Load pattern using load_pat
if ~isfile(pattern_path)
    error('Pattern file not found: %s', pattern_path);
end

try
    [frames, meta] = maDisplayTools.load_pat(pattern_path);
catch ME
    error('Failed to load pattern file: %s\nError: %s', pattern_path, ME.message);
end

% Get pattern dimensions from meta (rows/cols are total pixel dimensions)
pattern_rows = meta.rows;
pattern_cols = meta.cols;
